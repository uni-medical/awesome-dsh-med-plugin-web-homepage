...
export function ResearchPage() {
  ...
  const candidates = useMemo(() => entries.filter(entry => !ids.includes(entry.id) && `${entry.fullName} ${entry.description ?? ""}`.toLowerCase().includes(query.toLowerCase())).slice(0, 10), [entries, ids, query]);
