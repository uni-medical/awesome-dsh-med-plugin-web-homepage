...
export function HomePage() {
  ...
  const { locale: language, zh, toggleLocale } = useUiLanguage();
  ...
  const snapshotDate = new Intl.DateTimeFormat(zh ? "zh-CN" : "en", { dateStyle: "medium", timeZone: "UTC" }).format(new Date(catalog.generatedAt));
  ...
  return <main className="home"><div className="hero-image" style={{backgroundImage: `url(${heroImageUrl})`}} aria-hidden="true"/><div className="grid" /><nav><Brand/><div><Link to="/marketplace">{zh ? "组件市场" : "Marketplace"}</Link><button className="language-toggle" type="button" onClick={toggleLocale} aria-label={zh ? "Switch to English" : "切换中文"}>{zh ? "EN" : "中文"}</button><a href="https://github.com/uni-medical/awesome-dsh-med-plugin-web-homepage">GitHub</a></div></nav><section className="hero"><div><span className="eyebrow">{copy.eyebrow}</span><h1>{copy.title}</h1><p>{copy.desc}</p><button onClick={() => navigate("/marketplace")}>{copy.browse}</button></div><aside className="snapshot"><small>{copy.snapshot}</small><strong>{summary.total}</strong><span>{copy.records} · {snapshotDate}</span><hr/><div><b>{typeCount}</b> {copy.types}</div><div><b>{summary.medical}</b> {copy.medical}</div></aside></section><ScrollJourney language={language}/><section className="pillars"><article><b>01</b><h2>{copy.discover}</h2><p>{copy.discoverText}</p></article><article><b>02</b><h2>{copy.compare}</h2><p>{copy.compareText}</p></article><article><b>03</b><h2>{copy.trace}</h2><p>{copy.traceText}</p></article></section></main>;
}
