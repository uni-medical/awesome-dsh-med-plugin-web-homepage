...
function EntryActions({ entry, onSelect }: { entry: CatalogEntry; onSelect: (entry: CatalogEntry) => void }) {
  return <div className="result-actions"><button type="button" onClick={(event) => { event.stopPropagation(); onSelect(entry); }}>View details</button><CollectionPicker entryId={entry.id}/><a href={entry.repositoryUrl} onClick={event => event.stopPropagation()} target="_blank" rel="noopener noreferrer">Repository ↗</a></div>;
}
...
function TableView({ entries, selectedId, onSelect }: ResultProps) {
  return <div className="workbench-table-scroll"><table className="workbench-table"><thead><tr><th>Project</th><th>Status</th><th>Category</th><th>Domain</th><th>Stars</th><th>License</th><th>Language</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{entries.map(entry => <tr data-entry-id={entry.id} className={selectedId === entry.id ? "is-selected" : ""} key={entry.id} tabIndex={0} onClick={() => onSelect(entry)} onKeyDown={event => selectFromCard(event, entry, onSelect)}>
