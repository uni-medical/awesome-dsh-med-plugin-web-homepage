...
function EntryActions({ entry, onSelect }: { entry: CatalogEntry; onSelect: (entry: CatalogEntry) => void }) {
  return <div className="result-actions"><button type="button" onClick={() => onSelect(entry)}>View details</button><a href={entry.repositoryUrl} target="_blank" rel="noopener noreferrer">Repository ↗</a></div>;
}
...
function CardView({ entries, selectedId, onSelect }: ResultProps) {
  return <div className="repository-cards">{entries.map(entry => <article className={`repository-card${selectedId === entry.id ? " is-selected" : ""}`} key={entry.id}>
...
function TableView({ entries, selectedId, onSelect }: ResultProps) {
  return <div className="workbench-table-scroll"><table className="workbench-table"><thead><tr><th>Project</th><th>Status</th><th>Category</th><th>Domain</th><th>Stars</th><th>License</th><th>Language</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{entries.map(entry => <tr className={selectedId === entry.id ? "is-selected" : ""} key={entry.id}>
...
function GalleryView({ entries, selectedId, onSelect }: ResultProps) {
  return <div className="repository-gallery">{entries.map(entry => <article className={`gallery-card${selectedId === entry.id ? " is-selected" : ""}`} key={entry.id}>
