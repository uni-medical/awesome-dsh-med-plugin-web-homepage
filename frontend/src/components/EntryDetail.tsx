...
export function EntryDetail({ entry, onClose, focusOnOpen = false }: { entry?: CatalogEntry; onClose: () => void; focusOnOpen?: boolean }) {
  ...
  useEffect(() => { ref.current?.scrollTo({ top: 0 }); if (focusOnOpen) ref.current?.focus({ preventScroll: true }); }, [entry?.id, focusOnOpen]);
  ...
  return <aside className="entry-detail-pane" aria-labelledby="entry-title"><div ref={ref} className="detail-scroll" tabIndex={-1}>
